...
const correctPassword = process.env.NEXT_PUBLIC_CORDELY_PASSWORD
...
export default function Home() {
  ...
  useEffect(() => {
    const savedPassword = localStorage.getItem("cordely_password");
    if (savedPassword) {
  ...
  useEffect(() => {
    ...
    setOpenFlag(isValid);
    localStorage.setItem("cordely_password", password);
  }, [password, setOpenFlag]);

  // openFlag の変更を localStorage に保存（状態復元用）
  useEffect(() => {
    localStorage.setItem("cordely_open_flag", JSON.stringify(openFlag));
  }, [openFlag]);
